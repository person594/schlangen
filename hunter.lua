    ...
    function evade_hunt_dir()
        local evade_threshhold = 5
        local hunt_threshhold = 10
        ...
        for i, segment in pairs(findSegments (self.sight_radius, false)) do
            ...
            if math.abs(segment.d) <= 3*math.pi / 5 then -- evade
        ...
        log(string.format("Evade: %f\nHunt: %f", max_evade_urgency, max_hunt_urgency))
        ...
        elseif max_hunt_urgency > hunt_threshhold then
            log("hunt")
            return sum_hunt_dir / math.abs(sum_hunt_dir) * self.max_step_angle, true
        else
            return nil
        end
